...
public class FlightManager implements FlightController {
    ...
    public String takeOff() {
        ...
        for (Trip trip : this.airport.getTrips()) {
            ...
            for (Passenger passenger : trip.getFlyingMachine().getPassengers()) {
                try{
                    Field bags = passenger.getClass().getDeclaredField("bags");
                    bags.setAccessible(true);
                    String username = passenger.getUsername();
                    List<Passenger> passengers = airport.getPassengers();
                    for (Passenger passengerN : passengers) {
                        if (passengerN.getUsername().equals(passenger.getUsername())){
                            bags.set(passenger, passengerN.getBags());
                            break;
                        }
                    }
